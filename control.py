...
if clientID != -1:
    ...
    time.sleep(2)
    number, objecthandle = vrep.simxGetObjectHandle(
        clientID, 'Quadricopter_target', vrep.simx_opmode_blocking)
    while (a[0] == 0 and a[1] == 0 and a[2] == 0):
        es, a = vrep.simxGetObjectPosition(
            clientID, objecthandle, -1, vrep.simx_opmode_oneshot)
    es, rot = vrep.simxGetObjectOrientation(
        clientID, objecthandle, -1, vrep.simx_opmode_oneshot)
    print(rot)
    ...
    while True:
       # used try so that if user pressed other than the given key error will not be shown
        ...
        if keyboard.is_pressed("8"):
            ...
            a[1] += 0.05*math.cos(rot[2])
            a[0] += 0.05*math.sin(rot[2])
            vrep.simxSetObjectPosition(
                clientID, objecthandle, -1, a, vrep.simx_opmode_oneshot)
            print(a)
        ...
        if keyboard.is_pressed("4"):
            a[0] -= 0.05*math.cos(rot[2])
            a[1] -= 0.05*math.sin(rot[2])
            vrep.simxSetObjectPosition(
                clientID, objecthandle, -1, a, vrep.simx_opmode_oneshot)
            print(a)
        ...
        if keyboard.is_pressed("page up"):
            ...
            vrep.simxSetObjectOrientation(
                clientID, objecthandle, -1, rot, vrep.simx_opmode_oneshot)
